...
export default function LinkedIn({imageUrl,answer,month,page,post,setSuccess,dbId,forLi,setForLi,setForIg}) {
...
useEffect(() => {
  ...
  return ( <> 
    <button onClick={() => {setTime(time?false:true); setForLi(forLi?false:true); setForIg(false)}} className='bg-gradient-to-r from-blue-500 to-blue-700 px-2 mt-2 ml-2 rounded hover:scale-[1.1]'>Post on LinkedIn</button>

    {forLi && <div className='mt-2 bg-secondary px-4 py-2'>
          ...
          <input className='text-black rounded  ' type='datetime-local' onChange={(e) => setSelectedTime(e.target.value)}/>
            ...
            <label className='text-white'>Add hashtags</label>
             <textarea className='text-black rounded'  placeholder='Add hashtags'  onChange={(e) => {setHashtags(e.target.value)}}/>
                <button className='mt-4 rounded bg-gradient-to-r from-blue-500 to-blue-700 hover:cursor-pointer hover:scale-[1.1]'> Add hashtags </button>
            </form>
