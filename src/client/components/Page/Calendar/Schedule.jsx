...
<button disabled={btnDisabled} onMouseLeave={() => setOnhover(false)} onMouseEnter={() => setOnhover(true)} onClick={refreshData} className="bg-slate-700  outline hover:outline-2 outline-slate-400 text-white px-4 py-2 rounded-md mt-5 mb-10">Refresh Data</button>
...
{SwitchingPage && <div className=" flex-col  fixed top-0 w-[100vw] bg-slate-600 left-0 z-[100] flex items-center justify-center h-[100vh]">
  ...
  <p className="text-white text-2xl animate-bounce">Loading...</p>
      ...
      {level > 8 && dataItem.name !== 'Test' && <>
        <p>Pending: {matchingData.filter(matchingItem => matchingItem.client === dataItem.name && matchingItem.status !== "Approved" ).length} </p>
      </>}
