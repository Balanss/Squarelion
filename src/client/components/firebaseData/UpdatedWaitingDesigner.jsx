...
async function handleToDdesigner(){
  const date = new Date();
  ...
  if (!dcSnap.exists || !dcSnap.data().files) {
    data.files = '';
  }

  if (imageUrls[0] !== '') {
    data.img = img;
    data.img1 = imageUrls[0] || "";
    data.img2 = imageUrls[1] || "";
    data.img3 = imageUrls[2] || "";
    data.img4 = imageUrls[3] || ""; 
  }

  await dcRef.set(data, {merge:true});
...
const [sureYouWantToDelete, setSureYouWantToDelete] = useState(false)
  ...
  <Roles roles={roles} setRoles={setRoles} level={level} />
    <p  className='bg-yellow-500 w-[30vw] m-auto mt-2 p-2 mb-2 text-center cursor-pointer transition-transform transform-gpu hover:scale-110' onClick={handleData}> Designer </p>
...
<div className="absolute bg-slate-200 w-[25vw]  ">    
